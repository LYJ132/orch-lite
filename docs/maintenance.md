...
| Codex | Hooks-capable (manual trust) | Codex now ships a hooks engine (docs: [learn.chatgpt.com/docs/hooks](https://learn.chatgpt.com/docs/hooks), verified 2026-09-13): `SessionStart` (matcher on source: `startup`/`resume`/`clear`/`compact`) and synchronous `PreToolUse` (matcher on tool name; `spawn_agent` matches `Agent`) both exist, and plugins distribute hooks — Codex loads `hooks/hooks.json` in the plugin root by default, or a `hooks` entry in `.codex-plugin/plugin.json` (which this plugin now sets to `./hooks/hooks.json`, same file as the Claude/ZCode manifest). Both of orch-lite's hooks map directly: `session-init.py` emits `hookSpecificOutput.hookEventName: "SessionStart"` + `additionalContext` (exactly Codex's supported shape), and `dispatch-validate.py` denies via exit 2 + stderr (Codex's supported deny path for PreToolUse). What the user must do manually: trust the plugin's hooks — every non-managed hook requires explicit trust (`/hooks` in the CLI) and project-level hooks additionally require the project to be trusted (`[projects."<path>"] trust_level = "trusted"` in `~/.codex/config.toml`) — until then the hooks do not run and the skill stays in model-layer-discipline mode (six-rule MUST block + Supremacy clause; the skill remains safe without hooks). Tool-name caveat: Codex `spawn_agent` matches the `Agent` half of the `Agent|Task` matcher; ZCode `Task` is covered too, so the matcher works on both platforms. |
