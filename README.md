...
Parallelism is inappropriate when:

- tasks modify the same file;
- a later task strongly depends on an earlier task's result;
- tasks have a strict ordering requirement;
- parallelism introduces complex conflict handling.

Forcing parallelism in these cases only reduces efficiency. The standard for worthwhile parallelism is: **it removes human waiting without breaking correctness between tasks.**

---

## When It Fits

- Long code changes, refactoring, debugging, documentation generation;
- Writing tests and check tasks that can run in the background;
- Development of multiple independent features;
- Scenarios where continuous conversation is needed while agents work.

---

## When It Doesn't

- **Simple Q&A** — no background agent needed.
- **Very short tasks** — for a change that takes seconds, dispatch costs more than direct execution.
- **Strongly coupled tasks** — when a later task must wait for an earlier result, parallelism is pointless.
- **Tasks that cannot be safely isolated** — when multiple tasks must modify the same resource and conflicts are likely, parallelism should be used with care.

---

## Current Limitations

- Human decisions themselves can become the bottleneck;
- some work must be done by the main session;
- external searches and verification can still cause waiting;
- resource conflicts between parallel tasks remain possible;
- Git / worktree introduces some complexity;
- background tasks require additional state management;
- coding agents differ in their support for background execution, hooks, and lifecycles.

The goal is not zero waiting, but **not letting agent execution that could run in parallel block the human workflow.**

---

## Supported Environments

- **Claude Code / ZCode**: automatic hook registration.
- **Codex**: manual hook trust; without hooks enabled, it degrades to model-discipline mode.

Deployment and maintenance rules: `docs/maintenance.md`.
...
1. **Separate conversation from execution.** The main session handles understanding, discussion, and decisions; background agents handle code, files, tests, and debugging.
2. **The human workflow is not held hostage by agent execution time.** How long an agent runs is not how long the human waits.
3. **Keep the main session clean.** Focus on requirements, ideas, direction, questions, decisions, and feedback — not code, logs, tool calls, test output, or debugging traces.
4. **Human decision authority does not vanish with parallelism.** Background execution is not autonomous decision-making; important direction, review, and integration return to the human.
5. **Multi-agent is a means, not the goal.** When tasks don't need parallelism, multi-agent is not used for its own sake.
