...
**Two-layer agent model (agents.json > index)**: `agents.json` holds feature-independent agent definitions only (id/role/purpose/standard_acceptance/notes — what agents exist; it never names a feature). Feature-scoped bindings live in the index: each index entry carries `feature_id` plus an `agent` field referencing an agents.json id — which registered agent works which feature. Index entries reference, never define: an `agent` id absent from `agents.json` is rejected by `index create/update` (agents.json > index, enforced). Resolution order: role definition always from `agents.json`; feature binding always from the index.
