...
v3 validates the **fenced-JSON dispatch package inside the prompt text** with
HARD ENFORCEMENT: the platform's Agent `tool_input` only carries `description`
/ `prompt` / `subagent_type` / `run_in_background`, so the dispatch package
lives in the prompt as a ```json fenced block. The hook takes the first
```json block (fallback: any fenced block whose content contains `task_id`);
with no fenced block the call is **DENIED** — every Agent call from the main
session is a dispatch and must be re-sent with the fenced block. Calls whose
`run_in_background` is not exactly `true` are also **DENIED** — dispatches
must be background so the main session returns to the user immediately.
...
ignored when present. Malformed stdin / internal error → silent exit 0.

| # | Input | Expected | Actual |
|---|---|---|---|
| 1.1 | valid fenced JSON (all 4 required fields) + `run_in_background: true` | `{"decision":"allow"}` | ✅ |
| 1.2 | fenced JSON missing `objective` (+ background) | `{"decision":"deny","reason":"...Missing: objective"}` | ✅ |
| 1.3 | fenced JSON + `feature_id: "Bad_Branch!"` (+ background) | `{"decision":"deny","reason":"...feature_id must match [a-z0-9][a-z0-9._-]* (used as the feature/<feature_id> branch name)"}` | ✅ |
| 1.4 | prompt without any fenced block | `{"decision":"deny","reason":"Every Agent call from the main session is a dispatch: re-send the call with a ```json fenced block in the prompt containing task_id / role / objective / acceptance_criteria (+ optional feature_id)"}` | ✅ |
| 1.5 | fenced block containing malformed JSON (+ background) | `{"decision":"deny","reason":"dispatch fenced block is not valid JSON"}` | ✅ |
| 1.6 | fenced JSON incl. legacy `instance_id` field (+ background) | `{"decision":"allow"}` (instance_id retired, ignored) | ✅ |
| 1.7 | real-shaped Agent `tool_input` (description/prompt/subagent_type/run_in_background), no fenced JSON | deny (no-block reason) — the case that was dead under v1's marker sniffing | ✅ |
| 1.8 | malformed JSON on stdin | silent, exit 0, never block | ✅ |
| 1.9 | `tool_name: Task` alias, valid fenced JSON + background | `{"decision":"allow"}` | ✅ |
| 1.10 | fenced JSON + valid `feature_id: "payment"` + background | `{"decision":"allow"}` | ✅ |
| 1.11 | valid fenced JSON but `run_in_background` absent | `{"decision":"deny","reason":"Dispatches must run in the background (run_in_background=true) so the main session returns to the user immediately; re-send with run_in_background set to exactly true"}` | ✅ |
| 1.12 | valid fenced JSON but `run_in_background: false` | deny (foreground reason, as 1.11) | ✅ |
...
| 3.5 | SKILL.md with both contract sections (current shape) | additionalContext contains both "--- Request Routing (from SKILL.md) ---" and "--- Dispatch Package (MUST template) (from SKILL.md) ---", each section byte-identical to the SKILL.md text (verified programmatically: `extract_contract` output vs the slice between headings) | ⏳ re-run pending (see note) |
| 3.6 | SKILL.md without the "## Dispatch Package (MUST template)" section | fallback line for that section only, no crash, exit 0 | ⏳ re-run pending (see note) |

> Note (2026-09-12, impl-20260912-04): rows 3.5–3.6 cover the v3 five-section
> payload. The executor's shell was unavailable in this session, so they are
> logged as pending; re-run with:
> `printf '{}' | python3 hooks/session-init.py | python3 -c "import json,sys; ctx=json.load(sys.stdin)['additionalContext']; print('Routing OK' if '## Request Routing' in ctx else 'FAIL'); print('Dispatch OK' if '## Dispatch Package (MUST template)' in ctx else 'FAIL')"`
